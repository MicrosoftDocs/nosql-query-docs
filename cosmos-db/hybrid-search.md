...
# Hybrid search in Cosmos DB (in Azure and Fabric)

Cosmos DB (in Azure and Fabric) supports the hybrid search capability that combines vector search with full text search scoring (BM25) using the Reciprocal Rank Fusion (RRF) function.

## What is hybrid search?

Hybrid search uses the strengths of both vector-based and traditional keyword-based search methods to deliver more relevant and accurate search results. Hybrid search is easy to do in Cosmos DB due to the ability to store both metadata and vectors within the same document.

Hybrid search in Cosmos DB integrates two distinct search methodologies:
